...
class ConvBlock_pp(nn.Module):
    def __init__(self, i, o, ks=3, s=1, pd=1, bn='bn', relu=True):
        ...
        # passport
        self.passport_gamma = torch.randn((1, 3, 32, 32), device='cuda')
        self.passport_beta = torch.randn((1, 3, 32, 32), device='cuda')
    ...
    def forward(self, x):
        ...
        # passport
        gamma = self.encoder(self.conv(self.passport_gamma)).mean()
        beta = self.encoder(self.conv(self.passport_beta)).mean()
